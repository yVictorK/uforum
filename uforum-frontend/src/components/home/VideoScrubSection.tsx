...
export function VideoScrubSection() {
  ...
  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    if (videoRef.current && !isNaN(duration) && duration > 0 && videoRef.current.readyState >= 1) {
      // Faz o vídeo terminar o playback inteiro aos 50% do scroll
      // Terminando a animação entre o 1º e 2º texto. O resto do scroll só mudará a UI!
  ...
  return (
    // Reduzimos drasticamente a altura para 250vh. 
    // Assim o usuário não precisa scrolar a vida inteira para ver do começo ao fim.
    <section ref={containerRef} className="relative h-[250vh] w-full bg-zinc-950">
      <div className="sticky top-0 h-[100dvh] w-full overflow-hidden flex items-center justify-center">
        ...
        {/* Glass Gradient overlay for legibility */}
        <div className="absolute inset-0 bg-gradient-to-b from-zinc-950/90 via-zinc-950/50 to-zinc-950/90" />
        
        {/* First Hero Block (Logo ao abrir o site) */}
        <motion.div
           style={{
        ...
        >
          ...
          <h1 className="text-5xl md:text-7xl lg:text-[6rem] font-black leading-[0.95] tracking-tighter mb-6 text-white text-balance drop-shadow-2xl">
            O seu campus.<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-300">
              agora digital.
            </span>
          </h1>
          <p className="text-lg md:text-xl max-w-[45ch] mb-10 leading-relaxed text-zinc-300 font-medium text-balance drop-shadow-lg">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full justify-center">
            <Link href="/auth/register" className="btn-green w-full sm:w-auto text-base py-3.5 px-8 text-zinc-950 bg-emerald-400 hover:bg-emerald-300 shadow-glow-emerald">
              Acessar Fóruns <ArrowRight weight="bold" className="w-5 h-5 ml-1" />
            </Link>
          </div>
        </motion.div>

        {/* Second Step - Scroll Reveal */}
        <motion.div
           style={{
        ...
        >
           <h2 className="text-5xl md:text-7xl lg:text-[5rem] font-black leading-none text-white tracking-tight mb-4 drop-shadow-2xl">
             O campus de cima.
           </h2>
           <p className="text-zinc-200 text-lg md:text-xl font-medium max-w-[45ch] mx-auto drop-shadow-lg">
             Desvende o complexo universitário navegando suavemente pela plataforma construída por alunos.
           </p>
        </motion.div>

        {/* Third Step - Final Intro to App */}
        <motion.div
           style={{
             opacity: useTransform(smoothProgress, [0.7, 0.85, 1], [0, 1, 1]), // Mantém no final para linkar com a próxima seção
             y: useTransform(smoothProgress, [0.7, 1], [100, -50]),
             scale: useTransform(smoothProgress, [0.7, 1], [0.95, 1])
           }}
           className="absolute z-10 page-wrap w-full flex flex-col items-center justify-center text-center pointer-events-none px-4"
        >
           <h2 className="text-6xl md:text-8xl font-black text-transparent bg-clip-text bg-gradient-to-r from-zinc-100 to-zinc-400 tracking-tight mb-4 drop-shadow-[0_4px_24px_rgba(0,0,0,0.8)]">
             Integração Real.
           </h2>
           <p className="text-zinc-300 text-lg md:text-xl font-medium max-w-[45ch] mx-auto drop-shadow-lg">
